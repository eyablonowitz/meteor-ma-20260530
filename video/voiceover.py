...
SCENES = {
    ...
    "scene8": (
        "And if we know that delay at a few different spots, we can use the ruler "
        "to find the source. It turns out there's a whole network of sensors "
        "listening for things like earthquakes. Many are run by amateurs in their "
        "own backyards. And a bunch of them "
        "picked up our meteor boom! Crucially, each station records not just the "
        "sound, but the exact time it arrived. The boom reaches different sensors "
        "a few moments apart. Line up those arrival times, and you can trace the "
        "sound back to its source. Our program does the math, and we get an "
    ...
    "scene9": (
        "So \u2014 how big was it? The boom itself tells you. The same sensors "
        "that pinned down the source also caught the exact shape of the pressure "
        "wave \u2014 and the pitch of a blast works like a scale. A small bang "
        "cracks with a high, sharp snap. A giant one rumbles with a deep, slow "
        "note. Our boom rang the air with a low, two-second note \u2014 far too "
        "deep for a quarry blast, or a passing truck. Run those measurements "
        "through the physics, and you get a rough size: about a couple hundred "
        "tons of TNT. To feel that, picture a few hundred lightning bolts, all "
        "striking in the very same instant, miles above your head. Which is "
        "exactly why it rattled half of New England."
    ),
